...
% signal with period 4. The spectrum of DS are just spikes spaced by N/p * original sampling freq,
% where N is signal length. 
%%
figure; hold on
for p = 2:4
    DS = mod(time,p)==0;
    plot((-N/2):(N/2-1),fftshift(abs(fft(DS))),'DisplayName',['p = ',num2str(p)])
end
legend()
%%
close all